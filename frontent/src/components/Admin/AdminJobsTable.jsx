...
import toast from 'react-hot-toast';
import { Badge } from '../ui/badge';
import { setAllAdminJobs } from '../../redux/JobSlice'; // ✅ ADDED setAllAdminJobs
...
export default function AdminJobsTable() {
  ...
  const statusHandler = async (jobId, newStatus) => {
    try {
      const res = await axios.post(`${API_V1}/job/status/${jobId}/update`, { status: newStatus }, { withCredentials: true });
      if (res.data.success) {
        toast.success(res.data.message);
        
        // ✅ INSTANTLY UPDATE REDUX STATE (No refresh needed!)
