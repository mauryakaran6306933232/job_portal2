...
//         setLoading(true);
//         try {
//             const res = await axios.post(`http://localhost:8000/api/v1/application/${applicationId}/send-email`, 
//                 formData, 
...
export default function CustomEmailDialog({ open, setOpen, applicantEmail, applicantName, jobTitle, applicationId }) {
  ...
  const submitHandler = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`http://localhost:8000/api/v1/application/${applicationId}/send-email`, formData, { withCredentials: true });
      if (res.data.success) {
        toast.success("Email sent successfully!");
        setOpen(false);
      }
