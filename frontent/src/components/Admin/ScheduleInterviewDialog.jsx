...
//         setLoading(true);
//         try {
//             const res = await axios.post('http://localhost:8000/api/v1/interview/schedule', {
//                 jobId: jobId,
...
//                         <CalendarPlus size={20} className="text-indigo-600" /> Schedule Interview
//                     </DialogTitle>
//                     <p className="text-sm text-gray-500">Scheduling for <span className="font-semibold">{applicant?.fullname}</span></p>
//                 </DialogHeader>
...
export default function ScheduleInterviewDialog({ open, setOpen, applicant, jobId }) {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:8000/api/v1/interview/schedule', {
        jobId: jobId, applicantId: applicant?._id, scheduledAt: formData.scheduledAt, location: formData.location || "Online", notes: formData.notes
      }, { withCredentials: true });
