import { COMPANY_API_END_POINT, API_V1 } from '../utils/constant'; 
...
export default function CompanyProfile() {
  ...
  useEffect(() => {
    const fetchCompanyData = async () => {
      try {
        setLoading(true);
        const companyRes = await axios.get(`${COMPANY_API_END_POINT}/get/${id}`, { withCredentials: true }); // ✅ CHANGE
        if (companyRes.data.success) setCompany(companyRes.data.company);
        
        const jobsRes = await axios.get(`${API_V1}/company/${id}/jobs`, { withCredentials: true }); // ✅ CHANGE
